        ...
        >
          ...
          <el-table-column prop="unit" label="商品单位"></el-table-column>
            <el-table-column prop="stock" label="商品库存">
               <template slot-scope="scope">
              <el-tag type="success" v-if="scope.row.stock>500" size="mini" closable v-text="scope.row.stock+' '+scope.row.unit"></el-tag>
              <el-tag type="info" v-else-if="scope.row.stock>250" size="mini" closable v-text="scope.row.stock+' '+scope.row.unit"></el-tag>
              <el-tag type="warning" v-else-if="scope.row.stock>100" size="mini" closable v-text="scope.row.stock+' '+scope.row.unit"></el-tag>
              <el-tag type="warning" v-else-if="scope.row.stock==0" size="mini" closable v-text="'暂无库存'"></el-tag>
              <el-tag type="danger" v-else size="mini" closable v-text="scope.row.stock+' '+scope.row.unit"></el-tag>
            </template>
            </el-table-column>
      ...
      <el-dialog title="添加商品" :visible.sync="addDialogVisible" width="50%" @close="closeAddDialog">
        <span>
          ...
          >
            ...
            <el-form-item label="商品分类" prop="categoryKeys">
              <el-cascader
                :options="catetorys"
                clearable
                filterable
            ...
            <el-form-item label="商品规格" prop="model">
      ...
      >
        <span>
          ...
          >
            ...
            <el-form-item label="商品规格" prop="email">
  ...
  methods: {
      ...
      this.$refs.editRuleFormRef.validate(async valid => {
        if (!valid) {
          ...
          if (res.code == 200) {
            ...
            this.$message.error("商品信息更新失败:" + res.msg);
          }
    ...
    async edit(id) {
      ...
      if (res.code == 200) {
        ...
        return this.$message.error("商品信息编辑失败" + res.msg);
      }
      ...
      this.$refs.addRuleFormRef.validate(async valid => {
        if (!valid) {
          ...
          if (res.code == 200) {
            ...
            return this.$message.error("商品添加失败:" + res.msg);
          }
